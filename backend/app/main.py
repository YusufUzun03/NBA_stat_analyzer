...
# --- Yahoo Fantasy (stateless OAuth proxy; see app/yahoo.py) ----------------
...
def _bearer(authorization: str | None) -> str:
    if not authorization or not authorization.lower().startswith("bearer "):
        raise HTTPException(status_code=401, detail="Missing Yahoo access token.")
    return authorization.split(" ", 1)[1].strip()


@app.get("/api/yahoo/status")
def yahoo_status():
    """Whether Yahoo is set up on this backend. 'Connected' is per-browser."""
    return {"configured": yahoo.is_configured()}


@app.get("/api/yahoo/login")
def yahoo_login(return_: str = Query(..., alias="return")):
    """Start OAuth: validate the frontend return URL, then redirect to Yahoo."""
    if not yahoo.is_configured():
        raise HTTPException(status_code=503, detail="Yahoo not configured on the server.")
    if not yahoo.is_allowed_return(return_):
        raise HTTPException(status_code=400, detail="Return URL not allowed.")
    return RedirectResponse(yahoo.authorization_url(yahoo.make_state(return_)))


@app.get("/api/yahoo/callback")
def yahoo_callback(code: str = Query(None), state: str = Query(None), error: str = Query(None)):
    """Yahoo redirects here; verify state, exchange code, hand tokens to the SPA."""
    payload = yahoo.read_state(state)

    def back(fragment: dict):
        return RedirectResponse(f"{payload['r']}#{urlencode(fragment)}")
    ...
    def err_page(msg: str) -> HTMLResponse:
        return HTMLResponse(
            ...
            <div style="text-align:center;max-width:420px;padding:24px"><div style="font-size:40px">⚠️</div>
            <h2>{msg}</h2><p style="color:#9aa3b8">Close this tab and try again.</p></div>""",
            status_code=400,
        )

    if not payload:
        return err_page("Login expired or invalid. Please try again.")
    if error:
        return back({"yahoo_error": f"Yahoo denied access ({error})."})
    if not code:
        return back({"yahoo_error": "No authorization code returned."})
    try:
        tok = yahoo.exchange_code(code)
    except yahoo.YahooError as e:
        return back({"yahoo_error": str(e)})
    return back(tok)


@app.get("/api/yahoo/teams")
def yahoo_teams(authorization: str | None = Header(None)):
    """The user's NBA teams + rosters. Caller passes their access token."""
    token = _bearer(authorization)
    try:
        return {"teams": yahoo.fetch_user_teams(token)}
    except yahoo.YahooError as e:
        # 'expired' -> 401 so the frontend refreshes and retries.
        raise HTTPException(status_code=401 if str(e) == "expired" else 409, detail=str(e))


@app.post("/api/yahoo/refresh")
def yahoo_refresh(req: RefreshReq):
    try:
        return yahoo.refresh_token(req.refresh_token)
    except yahoo.YahooError as e:
